...
export class KanbanView extends BasesViewBase {
	...
	private columnParents: Record<string, string> = {};
	...
	private readViewOptions(): void {
		...
		try {
			...
			// Read columnParents for nested column hierarchy
			const columnParentsValue = this.config.get("columnParents");
			if (typeof columnParentsValue === "string" && columnParentsValue.trim()) {
				try {
					this.columnParents = JSON.parse(columnParentsValue);
				} catch {
					this.columnParents = {};
				}
			} else {
			...
			if (typeof columnPositionsValue === "string" && columnPositionsValue.trim()) {
				try {
					this.columnPositions = JSON.parse(columnPositionsValue);
				} catch {
	...
	/**
	 * Build a column tree from the columnParents mapping.
	 * Returns root column keys (those with no parent or whose parent is not in groups).
	 */
	private getRootColumnKeys(groups: Map<string, TaskInfo[]>): string[] {
		const allKeys = new Set(Array.from(groups.keys()));
		const roots: string[] = [];
		for (const key of allKeys) {
			const parent = this.columnParents[key];
			if (!parent || !allKeys.has(parent)) {
				roots.push(key);
			}
		}
		return roots;
	...
	/**
	 * Get child column keys for a given parent column key.
	 */
	private getChildColumnKeys(parentKey: string): string[] {
		const children: string[] = [];
		for (const [key, parent] of Object.entries(this.columnParents)) {
			if (parent === parentKey) {
				children.push(key);
			}
		}
	...
	private async renderFlat(
		...
		// Check if there are nested columns and add appropriate class
		const hasNestedColumns = Object.keys(this.columnParents).length > 0;
		if (hasNestedColumns) {
			this.boardEl.addClass("kanban-view__board--nested");
		} else {
			this.boardEl.removeClass("kanban-view__board--nested");
		}

		// Enable 2D layout class
		if (this.enable2DLayout) {
			this.boardEl.addClass("kanban-view__board--2d");
		} else {
			this.boardEl.removeClass("kanban-view__board--2d");
		...
		// Render root columns and their nested children
		for (const rootKey of rootKeys) {
			if (!orderedKeys.includes(rootKey)) continue;
			const rootTasks = groups.get(rootKey) || [];
			...
			this.sortScopeTaskPaths.set(
				this.getSortScopeKey(rootKey),
				rootTasks.map((task) => task.path)
			);

			// Create parent column
			const parentColumn = await this.createColumn(
				rootKey,
				rootTasks,
				visibleProperties,
				groupByPropertyId
			...
			if (this.boardEl) {
				this.boardEl.appendChild(parentColumn);
				parentColumn.addClass("kanban-view__column--parent");
			}
			renderedKeys.add(rootKey);

			// Render nested child columns
			const childKeys = this.getChildColumnKeys(rootKey).filter((k) => orderedKeys.includes(k));
			for (const childKey of childKeys) {
				const childTasks = groups.get(childKey) || [];
				...
				this.sortScopeTaskPaths.set(
					this.getSortScopeKey(childKey),
					childTasks.map((task) => task.path)
				);

				const childColumn = await this.createColumn(
					childKey,
					childTasks,
					visibleProperties,
					groupByPropertyId
				);
				if (this.boardEl && parentColumn) {
					parentColumn.appendChild(childColumn);
					childColumn.addClass("kanban-view__column--child");
				}
				renderedKeys.add(childKey);
			}
		}

		// Render any remaining unrendered columns (orphaned or with missing parents)
		for (const groupKey of orderedKeys) {
			if (renderedKeys.has(groupKey)) continue;
	...
	private async renderSwimLaneTable(
		...
		for (const [swimLaneKey, columns] of swimLanes) {
			...
			for (const columnKey of columnKeys) {
				...
				// Add parent/child class for nested columns
				if (this.columnParents[columnKey]) {
					cell.addClass("kanban-view__column--child");
				} else {
					cell.addClass("kanban-view__column--parent");
				}
	...
	private async createColumn(
		...
		if (this.enable2DLayout) {
			...
			if (pos) {
				column.style.gridRow = `${pos.r}`;
				column.style.gridColumn = `${pos.c}`;
			}
		}
	...
	private calculateGridPosition(clientX: number, clientY: number): { r: number; c: number } {
		...
		sortedTops.forEach((top, index) => {
			const rowCols = rowMap.get(top)!;
			const maxBottom = Math.max(...rowCols.map((el) => top + el.offsetHeight));
	...
	private async handle2DColumnDrop(draggedKey: string, r: number, c: number): Promise<void> {
		...
		const resolve = (row: number, col: number, keyToIgnore: string) => {
			const occupiedKey = Object.keys(newPositions).find(
				(k) => k !== keyToIgnore && newPositions[k].r === row && newPositions[k].c === col
			);
		...
		resolve(r, c, draggedKey);
		newPositions[draggedKey] = { r, c };
	...
	private async saveColumnPositions(): Promise<void> {
		if (!this.config || typeof this.config.set !== "function") return;
		try {
			this.config.set("columnPositions", JSON.stringify(this.columnPositions));
		} catch (e) {
			tasknotesLogger.error("[KanbanView] Failed to save column positions:", {
				category: "configuration",
				operation: "save-column-positions",
				error: e,
			});
		}
	}
